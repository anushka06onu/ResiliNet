...
def run_gradual_congestion():
    ...
    log_experiment_event(results_dir, "congestion_injected_at", {"stage": 1, "delay_ms": phase1_delay_ms, "loss_percent": phase1_loss_pct})
    s1 = net.get('s1')
    s1.cmd(f'tc qdisc change dev s1-eth3 root netem delay {phase1_delay_ms}ms loss {phase1_loss_pct}%')

    time.sleep(duration // 3)
    info(f'*** Worsening congestion (delay: {phase2_delay_ms}ms, loss: {phase2_loss_pct}%)...\n')
    log_experiment_event(results_dir, "congestion_worsened_at", {"stage": 2, "delay_ms": phase2_delay_ms, "loss_percent": phase2_loss_pct})
    s1.cmd(f'tc qdisc change dev s1-eth3 root netem delay {phase2_delay_ms}ms loss {phase2_loss_pct}%')
