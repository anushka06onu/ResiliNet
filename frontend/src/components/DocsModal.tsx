      ...
      <div className="glass-panel w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in shadow-[0_0_50px_rgba(16,185,129,0.1)]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-700/50 bg-slate-800/80">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white tracking-wide">Project Documentation & Metrics</h2>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-8 bg-slate-900/50 text-slate-300">
          ...
          <section>
            <h3 className="text-lg font-semibold text-emerald-400 mb-3 border-b border-slate-700/50 pb-2">2. Datasets & Topologies</h3>
            <ul className="space-y-2 list-disc list-inside">
              <li><strong className="text-white">Topology Data:</strong> Sourced from the <span className="text-indigo-400">SNDlib (Survivable Network Design Library)</span> dataset to simulate realistic, real-world backbone network structures.</li>
              <li><strong className="text-white">Traffic Generation:</strong> Priority-based traffic generation using <span className="text-indigo-400">Mininet</span> and <span className="text-indigo-400">iperf</span> (Telemedicine/Critical vs Background traffic).</li>
              <li><strong className="text-white">Telemetry Extraction:</strong> Custom Python collectors hooking into <span className="text-indigo-400">Open vSwitch (OVS)</span> to extract port statistics every 1 second.</li>
            </ul>
          </section>

          <section>
            <h3 className="text-lg font-semibold text-emerald-400 mb-3 border-b border-slate-700/50 pb-2">3. Data Pipeline & Implementation</h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm leading-relaxed">We trained a <strong>LightGBM</strong> binary classifier to predict future SLA violations (congestion probability). We integrated <strong>SHAP (SHapley Additive exPlanations)</strong> to map exact feature contributions to predictions, rendering the model fully explainable rather than a black box.</p>
              </div>
              <div>
                <h4 className="text-white font-medium mb-1">Predictive Routing Engine</h4>
                <p className="text-sm leading-relaxed">Instead of reactive shortest-path routing, we implemented a predictive <strong>Dijkstra's Algorithm</strong> via NetworkX. Link weights are inflated dynamically based on the AI's predicted congestion probability, routing critical traffic around bottlenecks before they occur.</p>
              </div>
            </div>
          </section>
