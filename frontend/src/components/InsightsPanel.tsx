    ...
    return (
      <div className="w-full h-full glass-panel flex flex-col items-center justify-center text-slate-400 p-8 text-center">
        ...
        <p className="text-sm mt-2">
          Click on a network node or link on the map to view real-time telemetry and predictive AI
          insights.
        </p>
      </div>
    );
        ...
        {!isEdge ? (
          <div className="space-y-4">
            ...
            <div className="glass-card p-4 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-slate-200 capitalize font-medium bg-slate-700/50 px-2 py-0.5 rounded">
                  {selectedElement.data.type || 'Host'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-emerald-400 text-sm font-medium glow-text-green">
                    Online
                  </span>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div
                className={`glass-card p-4 relative overflow-hidden ${isCongested && !loading ? 'glow-border-red border-red-500/50 bg-red-950/20' : 'border-slate-700/40'}`}
              >
                ...
                <div className="relative flex justify-between items-center">
                  ...
                  {loading ? (
                    ...
                    <span
                      className={`text-2xl font-bold font-mono tracking-tight ${isCongested ? 'text-red-400 glow-text-red' : 'text-emerald-400 glow-text-green'}`}
                    >
                      {(prob * 100).toFixed(1)}%
                    </span>
                  )}
                ...
                {loading ? (
                    ...
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="flex flex-col gap-2">
                        ...
                        <div className="h-1.5 bg-slate-800 rounded w-full">
                          <div className="h-full bg-slate-700/50 rounded w-1/2 animate-pulse"></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-4">
                    <p className="text-xs text-slate-400 mb-4 leading-relaxed">
                      Features driving the risk prediction. Positive values increase congestion
                      risk.
                    </p>
                    {insight.explain.features.map((f: any, idx: number) => {
                      ...
                      const maxAbsShap = Math.max(
                        ...insight.explain.features.map((fx: any) =>
                          Math.abs(fx.shap_contribution),
                        ),
                        0.1,
                      );
                      const width = Math.min(
                        100,
                        Math.max(5, (Math.abs(f.shap_contribution) / maxAbsShap) * 100),
                      );

                      return (
                        <div key={idx} className="relative group">
                          <div className="flex justify-between text-xs mb-1.5">
                            <span className="text-slate-300 font-mono truncate w-32" title={f.name}>
                              {f.name}
                            </span>
                            <span
                              className={`font-mono font-medium ${isPositive ? 'text-red-400' : 'text-emerald-400'}`}
                            >
                              {isPositive ? '+' : ''}
                          ...
                          <div className="w-full bg-slate-900 rounded-full h-2 flex items-center">
                            <div className="flex-1 flex justify-end h-full">
                              {!isPositive && (
                                <div
                                  className="bg-gradient-to-l from-emerald-400 to-emerald-600 h-full rounded-l-full shadow-[0_0_8px_rgba(16,185,129,0.5)] transition-all duration-1000"
                                  style={{ width: `${width}%` }}
                                ></div>
                              )}
                            </div>
                            ...
                            <div className="flex-1 h-full">
                              {isPositive && (
                                <div
                                  className="bg-gradient-to-r from-red-400 to-red-600 h-full rounded-r-full shadow-[0_0_8px_rgba(239,68,68,0.5)] transition-all duration-1000"
                                  style={{ width: `${width}%` }}
                                ></div>
                              )}
                            </div>
