      ...
      <header className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Real-time Quality of Service tracking for active application flows.
          </p>
        </div>
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="bg-slate-900 border border-slate-700 text-sm rounded pl-9 pr-3 py-1.5 focus:outline-none focus:border-emerald-500 text-white"
            />
          </div>
            ...
            {flows.map((flow: any) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-mono text-xs">
                  {flow.current_path ? flow.current_path.join(' → ') : 'Unknown'}
                </td>
                <td className="px-4 py-3">
                  {flow.metrics?.latency_ms ? flow.metrics.latency_ms + 'ms' : 'Pending'}
                </td>
                <td className="px-4 py-3">
                  {flow.metrics?.loss_percent ? flow.metrics.loss_percent + '%' : 'Pending'}
                </td>
                <td className="px-4 py-3">
                  <span
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-8 ${parseInt(flow.risk || '0') > 50 ? 'text-amber-400' : 'text-slate-400'}`}
                    >
                      {flow.risk || '0%'}
                    </span>
                    <div className="flex-1 h-1.5 bg-slate-800 rounded overflow-hidden">
                      <div
                        className={`h-full ${parseInt(flow.risk || '0') > 80 ? 'bg-red-500' : parseInt(flow.risk || '0') > 50 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                        style={{ width: flow.risk || '0%' }}
                      ></div>
                    </div>
                  </div>
                </td>
