...
import { Activity, ShieldCheck, AlertTriangle, Route, Clock } from 'lucide-react';
  ...
  useEffect(() => {
    const checkStaleness = () => {
      if (latestTelemetry?.timestamp) {
        const age = Date.now() - new Date(latestTelemetry.timestamp).getTime();
        setIsStale(age > 10000); // 10 seconds without updates = stale
      } else {
        setIsStale(false);
      }
    };
    ...
    <div className="p-6 h-full overflow-y-auto">
      <header className="mb-8">
        ...
        <div className="flex gap-4 text-sm items-center">
          ...
          <span className="px-2 py-1 bg-slate-800 rounded text-slate-300">
            Experiment:{' '}
            <strong className="text-white">{latestTelemetry?.experiment_id || 'None'}</strong>
          </span>
          <span className="px-2 py-1 bg-slate-800 rounded text-slate-300">
            Connections: <strong className="text-emerald-400">{activeConnections}</strong>
          </span>

          {isStale && (
            <span className="px-2 py-1 bg-red-900/30 text-red-400 border border-red-500/50 rounded flex items-center gap-1">
              <Clock size={14} /> Data Stale
            </span>
          )}
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-emerald-500 mt-1 flex items-center gap-1">
              <ShieldCheck size={12} /> Monitoring
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-6 h-96">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 flex flex-col">
          <h3 className="text-slate-300 font-medium mb-4">
            Alert Lifecycle (Flow 1: Telemedicine)
          </h3>
          <div className="flex-1 overflow-y-auto space-y-2 relative pl-4 border-l-2 border-slate-700 ml-2">
            <div className="relative">
              ...
              <p className="text-sm text-slate-300">
                Link s2-s4 congestion risk reached 85/100. Strongest contributor: Rapid traffic
                growth.
              </p>
            </div>
            ...
            <div className="relative mt-4">
              ...
              <p className="text-xs font-bold text-indigo-400">
                14:31:59 - Route Proposed & Safety Checked
              </p>
              <p className="text-sm text-slate-300">
                Alternative path S1-S3-S7 calculated via Dijkstra. Safety checks passed (No loops
                detected).
              </p>
            </div>
            ...
            <div className="relative mt-4">
              ...
              <p className="text-sm text-slate-300">
                OpenFlow rules injected via `ovs-ofctl`. Original route retained as backup.
              </p>
            </div>
            ...
            <div className="relative mt-4">
              ...
              <p className="text-xs font-bold text-emerald-400">
                14:32:01 - Route Verification (Demo)
              </p>
              <p className="text-sm text-slate-300">
                In a live lab, port counters will verify if traffic shifted and latency improved.
              </p>
            </div>
          </div>
        </div>
